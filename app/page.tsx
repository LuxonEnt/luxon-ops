Yes — replace **all of `app/page.tsx`** with this full corrected code:
...
function contractorToDb(contractor: Partial<Contractor>) {
  return {
    ...
    name: contractor.name || `${contractor.firstName || ""} ${contractor.lastName || ""}`.trim(),
...
export default function LuxonOpsDashboard() {
  ...
  const filteredRows = rows.filter((row) => {
    ...
    return (
      !term ||
      `${row.contractor?.name} ${row.event?.name} ${row.position} ${row.event?.venue}`
        .toLowerCase()
        .includes(term)
    );
  });
  ...
  const approvedContractorInvoiceRows = rows.filter(
    (row) =>
      ...
      row.contractorId === selectedInvoiceContractorId &&
      row.approved
  );
  ...
  const invoiceRegularHours = contractorInvoiceLines.reduce(
    (sum, row) => sum + row.regularHours,
    0
  );
  const invoiceOvertimeHours = contractorInvoiceLines.reduce(
    (sum, row) => sum + row.overtimeHours,
    0
  );
  const invoiceTotal = contractorInvoiceLines.reduce(
  ...
  async function addContractor() {
    if (
      !newContractor.firstName.trim() ||
      !newContractor.lastName.trim() ||
      !newContractor.phone.trim() ||
      !newContractor.email.trim()
    ) {
      return alert("First name, last name, phone, and email are required.");
    }

    const skills = newContractor.skillsText
      .split(/[\n,]+/)
    ...
    const { error } = await supabase.from("contractors").insert(
      contractorToDb({
        ...
        name: `${newContractor.firstName} ${newContractor.lastName}`.trim(),
        ...
        emergencyContactName: newContractor.emergencyContactName,
        emergencyContactPhone: newContractor.emergencyContactPhone,
  ...
  async function addAssignment() {
    ...
    const datesToSchedule =
      datesFromMultiField.length > 0
        ? datesFromMultiField
        : newAssignment.workDate
        ? [newAssignment.workDate]
        : [];
    ...
    const createdAssignments = datesToSchedule.map((date) =>
      assignmentToDb({
  ...
  async function updateEvent(id: number, field: keyof EventItem, value: string) {
    ...
    const updated = {
      ...current,
      [field]: field === "geofenceRadiusFeet" ? Number(value || 0) : value,
    } as EventItem;
  ...
  async function deleteEvent(id: number) {
    ...
    if (selectedEventId === id) {
      setSelectedEventId(events.find((event) => event.id !== id)?.id || 0);
    }
  ...
  function getCurrentPosition(): Promise<GeolocationPosition> {
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) {
        reject(new Error("Geolocation is not available on this phone/browser."));
        return;
      }
  ...
  async function geofenceClock(id?: number, type: "in" | "out" = "in") {
    ...
    if (!event.latitude || !event.longitude) {
      setGeoMessage(
        "This event needs latitude and longitude before geofence clock-in can work. Add it in Schedule > Edit Event."
      );
      return;
    }
    ...
    try {
      ...
      const locationStamp = `${userLat.toFixed(6)},${userLon.toFixed(6)} (${Math.round(
        distance
      )} ft from event)`;

      if (distance > radius) {
        setGeoMessage(
          `Clock-${type} blocked. You are about ${Math.round(
            distance
          )} ft away. Allowed radius is ${radius} ft.`
        );
        return;
      }
      ...
      setGeoMessage(
        `Clock-${type} approved. Location verified within ${Math.round(distance)} ft of event.`
      );
    } catch (error: any) {
      setGeoMessage(
        error?.message ||
          "Location permission failed. Contractor must allow location access to clock in/out."
      );
    }
  }
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-5 md:px-8 md:py-8">
        ...
        {loading && (
          <div className="mb-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-100">
            Loading saved data from Supabase...
          </div>
        )}
            ...
            <div className="space-y-5">
              <GlassCard>
                ...
                <div className="rounded-3xl border border-amber-400/20 bg-gradient-to-br from-amber-400/15 to-white/[0.03] p-5">
                  <h3 className="text-xl font-bold">{todayJob?.event?.name || "No job scheduled"}</h3>
              ...
              <GlassCard>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  <Select
                    ...
                    onChange={(v) => setNewAssignment({ ...newAssignment, eventId: Number(v) })}
                    options={events.map((e) => ({ value: e.id, label: e.name }))}
                  />
                  <Select
                    label="Contractor"
                    value={newAssignment.contractorId}
                    onChange={(v) => setNewAssignment({ ...newAssignment, contractorId: Number(v) })}
                    options={contractors.map((c) => ({ value: c.id, label: c.name }))}
                  />
                  <Input
                    label="Position"
                    value={newAssignment.position}
                    onChange={(v) => setNewAssignment({ ...newAssignment, position: v })}
                  />
                  <Input
                    label="Single Work Date"
                    type="date"
                    value={newAssignment.workDate}
                    onChange={(v) => setNewAssignment({ ...newAssignment, workDate: v })}
                  />
                  <Input
                    label="Call Time"
                    type="time"
                    value={newAssignment.callTime}
                    onChange={(v) => setNewAssignment({ ...newAssignment, callTime: v })}
                  />
                  <Input
                    label="Rate"
                    type="number"
                    value={newAssignment.rate}
                    onChange={(v) => setNewAssignment({ ...newAssignment, rate: v })}
                  />
                  ...
                  <div className="md:col-span-3">
                    <label className="block">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewAssignment({ ...newAssignment, workDates: e.target.value })
                        }
...
2026-05-17`}
                    ...
                    <p className="mt-2 text-xs text-zinc-500">
                      Use this when a contractor is working multiple days on the same event. The app will create one assignment per date so communication stays clear.
                    </p>
                  </div>
                  ...
                  <Select
                    ...
                    onChange={(v) =>
                      setNewAssignment({ ...newAssignment, rateType: v as "day" | "hour" })
                    }
                    options={[
            ...
            <GlassCard>
              ...
              <div className="space-y-3">
                ...
                <Input label="Address for Google Maps" value={newEvent.address} onChange={(v) => setNewEvent({ ...newEvent, address: v })} />
                <Input label="Latitude" value={newEvent.latitude} onChange={(v) => setNewEvent({ ...newEvent, latitude: v })} />
                <Input label="Longitude" value={newEvent.longitude} onChange={(v) => setNewEvent({ ...newEvent, longitude: v })} />
                <Input
                  label="Allowed Clock-In Radius Feet"
                  type="number"
                  value={newEvent.geofenceRadiusFeet}
                  onChange={(v) => setNewEvent({ ...newEvent, geofenceRadiusFeet: v })}
                />
            ...
            <div className="lg:col-span-2">
              <GlassCard>
                <SectionHeader
                  ...
                  subtitle="Edit contractor name, role, phone, email, rate, and rate type."
                />
                <div className="grid gap-4 md:grid-cols-2">
                  {contractors.map((contractor) => (
                    <EditableContractorCard
                      ...
                      onChange={(field, value) => updateContractor(contractor.id, field, value)}
                    />
                  ))}
                </div>
              </GlassCard>
              ...
              <div className="space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  <Input
                    ...
                    onChange={(v) => setNewContractor({ ...newContractor, firstName: v })}
                  />
                  <Input
                    label="Last Name *"
                    value={newContractor.lastName}
                    onChange={(v) => setNewContractor({ ...newContractor, lastName: v })}
                  />
                </div>

                <Input label="Primary Role" value={newContractor.role} onChange={(v) => setNewContractor({ ...newContractor, role: v })} />
                ...
                <Input label="Company" value={newContractor.company} onChange={(v) => setNewContractor({ ...newContractor, company: v })} />

                <div className="grid gap-3 md:grid-cols-2">
                  <Input label="City" value={newContractor.city} onChange={(v) => setNewContractor({ ...newContractor, city: v })} />
                  <Input label="State" value={newContractor.state} onChange={(v) => setNewContractor({ ...newContractor, state: v })} />
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Input
                    ...
                    onChange={(v) =>
                      setNewContractor({ ...newContractor, emergencyContactName: v })
                    }
                  />
                  <Input
                    ...
                    onChange={(v) =>
                      setNewContractor({ ...newContractor, emergencyContactPhone: v })
                    }
                  />
                </div>
                ...
                <label className="block">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewContractor({ ...newContractor, skillsText: e.target.value })
                    }
...
A1, A2, V1, V2, L1, L2, Stagehand, LED Tech, Playback, RF Tech`}
                ...
                <Input
                  ...
                  onChange={(v) => setNewContractor({ ...newContractor, rate: v })}
                />

                <Select
                  label="Rate Type"
                  value={newContractor.rateType}
                  onChange={(v) => setNewContractor({ ...newContractor, rateType: v as "day" | "hour" })}
                  options={[
                    { value: "day", label: "Day Rate" },
                    { value: "hour", label: "Hourly" },
                  ]}
            ...
            <GlassCard>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-4">
                  ...
                  <p className="text-3xl font-bold text-emerald-300">{money(invoiceTotal)}</p>
                ...
                <div className="rounded-3xl border border-white/10 bg-black/25 p-4 text-sm text-zinc-300">
                  <p>
                    ...
                    <span className="font-bold text-white">{invoiceRegularHours.toFixed(2)}</span>
                  </p>
                  <p>
                    Overtime hours:{" "}
              ...
              <GlassCard>
                ...
                >
                  <div className="mb-8 flex items-start justify-between border-b border-slate-200 pb-6">
                    <div>
                      <p className="text-2xl font-bold text-slate-950">Luxon Entertainment LLC</p>
                      <p className="text-slate-500">Contractor Invoice Approval Record</p>
                      <p className="mt-3 text-xs uppercase tracking-wider text-slate-400">Bill To</p>
                  ...
                  <div className="mb-6 grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-slate-200 p-4">
                      <p className="text-xs uppercase tracking-wider text-slate-400">Contractor</p>
                    ...
                    <div className="rounded-2xl border border-slate-200 p-4">
                      <p className="text-xs uppercase tracking-wider text-slate-400">Event</p>
                  ...
                  <div className="mt-8 rounded-2xl border border-slate-200 p-4 text-xs text-slate-500">
                    <p className="font-semibold text-slate-700">System Verification Note</p>
                    <p>
                      This invoice only includes approved assignment records. Pending or unapproved
                      hours are excluded. Totals are calculated from clock-in/out records, break
                      deductions, management approval status, configured rate, and overtime rules.
                    </p>
                  </div>
                </div>
              </GlassCard>
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          {isEditing ? (
            <Input label="Event Name" value={event.name} onChange={(v) => onChange("name", v)} />
          ) : (
            <>
              <h3 className="text-xl font-bold">{event.name}</h3>
              <p className="text-sm text-zinc-400">{event.client}</p>
            </>
          )}
        </div>
      ...
      {isEditing ? (
        <div className="grid gap-3 md:grid-cols-2">
          <Input label="Client" value={event.client} onChange={(v) => onChange("client", v)} />
          <Input label="Venue" value={event.venue} onChange={(v) => onChange("venue", v)} />
          <Input
            label="Address for Google Maps"
            value={event.address}
            onChange={(v) => onChange("address", v)}
          />
          <Input label="Latitude" value={event.latitude} onChange={(v) => onChange("latitude", v)} />
          <Input
            label="Longitude"
          ...
          />
          <Input
            label="Allowed Radius Feet"
            type="number"
            value={String(event.geofenceRadiusFeet)}
            onChange={(v) => onChange("geofenceRadiusFeet", v)}
          />
          <Input
            label="Start Date"
            type="date"
          ...
          />
          <textarea
            className="min-h-[100px] rounded-2xl border border-white/10 bg-black/30 p-3 text-sm text-white outline-none md:col-span-2"
            value={event.notes}
        ...
        <div className="grid gap-3 text-sm md:grid-cols-2">
          ...
          <InfoLine
            icon={<Crosshair className="h-4 w-4" />}
            label="Geofence"
            value={`${event.latitude}, ${event.longitude} · ${event.geofenceRadiusFeet} ft`}
          />
          <InfoLine
            icon={<CalendarDays className="h-4 w-4" />}
            label="Dates"
            value={`${dateLabel(event.startDate)} - ${dateLabel(event.endDate)}`}
          />
          <a
...
function EditableContractorCard({
  ...
  onSave: () => void;
  onDelete: () => void;
  onChange: (field: keyof Contractor, value: string) => void;
}) {
  return (
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      ...
      {isEditing ? (
        <div className="grid gap-3">
          ...
          />
          <Input label="Phone" value={contractor.phone} onChange={(v) => onChange("phone", v)} />
          <Input label="Email" value={contractor.email} onChange={(v) => onChange("email", v)} />
          <Input
            label="Company"
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Input label="City" value={contractor.city} onChange={(v) => onChange("city", v)} />
            <Input
              label="State"
          ...
          <Input
            label="Rate"
            type="number"
            value={String(contractor.rate)}
            onChange={(v) => onChange("rate", v)}
          />

          <Select
            label="Rate Type"
            value={contractor.rateType}
        ...
        <div className="mt-5 space-y-3 text-sm">
          <InfoLine
            icon={<DollarSign className="h-4 w-4" />}
            label="Default Rate"
            value={`${money(contractor.rate)} / ${contractor.rateType}`}
          />
          <InfoLine
            icon={<Phone className="h-4 w-4" />}
            label="Phone"
            value={contractor.phone || "Not added"}
          />
          <InfoLine
          ...
          <div className="rounded-2xl bg-black/20 p-3">
            <div className="text-xs text-zinc-500">Skills / Abilities</div>
            <div className="mt-2 flex flex-wrap gap-2">
              {contractor.skills?.length ? (
                ...
                <span className="text-zinc-400">No skills added</span>
              )}
            </div>
          </div>
        </div>
...
function EditableAssignmentCard({
  ...
  onSave: () => void;
  onDelete: () => void;
  onChange: (field: keyof Assignment, value: string | number | boolean) => void;
  showPayrollActions?: boolean;
  onToggle?: (id: number, key: "confirmed" | "approved" | "paid") => void;
}) {
  return (
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h3 className="text-xl font-bold">{row.contractor?.name}</h3>
          <p className="text-sm text-zinc-400">
            {row.position} · {row.event?.name}
          </p>
          <p className="mt-1 font-bold text-amber-300">
            {money(row.total)} · {row.hours.toFixed(2)} hrs
          </p>
        </div>
      ...
      {isEditing ? (
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <Select
            ...
            onChange={(v) => onChange("contractorId", v)}
            options={contractors.map((c) => ({ value: c.id, label: c.name }))}
          />
          <Input
            label="Position"
          ...
          <Input
            ...
            type="date"
            value={row.workDate}
            onChange={(v) => onChange("workDate", v)}
          />
          <Input
            label="Call Time"
            type="time"
            value={row.callTime}
            onChange={(v) => onChange("callTime", v)}
          />
          <Input
            label="Clock In"
            type="time"
            value={row.clockIn}
            onChange={(v) => onChange("clockIn", v)}
          />
          <Input
            label="Clock Out"
            type="time"
            value={row.clockOut}
            onChange={(v) => onChange("clockOut", v)}
          />
          <Input
            label="Break Hours"
            type="number"
            value={String(row.breakHours)}
            onChange={(v) => onChange("breakHours", v)}
          />
          <Input
            label="Rate"
            type="number"
        ...
        <div className="grid gap-3 text-sm md:grid-cols-4">
          <InfoLine
            icon={<CalendarDays className="h-4 w-4" />}
            label="Date"
            value={dateLabel(row.workDate)}
          />
          <InfoLine
            icon={<Clock className="h-4 w-4" />}
            label="Call / Clock"
            value={`${timeLabel(row.callTime)} · ${row.clockIn || "--"}-${row.clockOut || "--"}`}
          />
          <InfoLine
            icon={<DollarSign className="h-4 w-4" />}
            label="Rate"
            value={`${money(row.rate)} / ${row.rateType}`}
          />
          <InfoLine
            icon={<MapPin className="h-4 w-4" />}
            label="Clock Location"
            value={row.clockInLocation || "Not verified yet"}
          />
        </div>
      )}

      {showPayrollActions && onToggle && (
        <div className="mt-4 flex flex-wrap gap-2">
          <SmallButton onClick={() => onToggle(row.id, "confirmed")}>
            Toggle Confirmed
          </SmallButton>
          <SmallButton onClick={() => onToggle(row.id, "approved")}>
            Toggle Approved
          </SmallButton>
          <SmallButton onClick={() => onToggle(row.id, "paid")}>Toggle Paid</SmallButton>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-black/20 p-3">
      ...
      <div>
        ...
        <div className="break-words font-medium text-zinc-100">{value}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        placeholder="Search crew, event, role..."
        className="h-12 w-full rounded-2xl border border-white/10 bg-black/30 pl-11 pr-4 text-sm text-white outline-none transition placeholder:text-zinc-600 focus:border-amber-400/60 md:w-80"
      />
    </div>
  );
}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-12 w-full rounded-2xl border border-white/10 bg-black/30 px-4 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-amber-400/60"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="h-12 w-full rounded-2xl border border-white/10 bg-black/30 px-4 text-sm text-white outline-none focus:border-amber-400/60"
      >
        {options.map((option) => (
...
function GoldButton({
  ...
  return (
    <button
      onClick={onClick}
      className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-amber-300 to-yellow-600 px-5 font-semibold text-black shadow-lg shadow-amber-500/20 transition hover:scale-[1.01]"
    >
      {children}
    </button>
  );
...
function MessageBox({ text }: { text: string }) {
  return (
    <div className="mb-4 rounded-3xl border border-white/10 bg-black/40 p-5 text-sm leading-7 text-zinc-200">
      {text}
    </div>
  );
}
            ...
            <tr key={row.id} className="border-b border-white/5 last:border-0">
              ...
              <td className="py-4">
                <div className="flex flex-wrap gap-2">
                  ...
                  <IconButton label="Edit" onClick={() => onEdit(row.id)}>
                    <Pencil className="h-4 w-4" />
                  </IconButton>
                  <IconButton label="Delete" onClick={() => onDelete(row.id)} danger>
                    <Trash2 className="h-4 w-4" />
                  </IconButton>
                </div>
              </td>
...
After that, hard refresh the site and test the page again.
