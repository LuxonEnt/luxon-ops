...
export default function LuxonOpsDashboard() {
  ...
  const [newContractor, setNewContractor] = useState({ firstName: "", lastName: "", role: "", phone: "", email: "", company: "", city: "", state: "", emergencyContactName: "", emergencyContactPhone: "", notes: "", skillsText: "", rate: "", rateType: "day" as "day" | "hour" });
  ...
  async function addContractor() {
    if (!newContractor.firstName.trim() || !newContractor.lastName.trim() || !newContractor.phone.trim() || !newContractor.email.trim()) {
    ...
    setNewContractor({ firstName: "", lastName: "", role: "", phone: "", email: "", company: "", city: "", state: "", emergencyContactName: "", emergencyContactPhone: "", notes: "", skillsText: "", rate: "", rateType: "day" });
    loadData();
  }
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white overflow-hidden">
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-5 md:px-8 md:py-8">
        ...
        {activeTab === "crew" && <div className="grid gap-5 lg:grid-cols-3"><div className="lg:col-span-2"><GlassCard><SectionHeader icon={<Users className="h-6 w-6" />} title="Editable Crew Roster" subtitle="Edit contractor name, role, phone, email, rate, and rate type." /><div className="grid gap-4 md:grid-cols-2">{contractors.map((contractor) => <EditableContractorCard key={contractor.id} contractor={contractor} isEditing={editingContractorId === contractor.id} onEdit={() => setEditingContractorId(contractor.id)} onCancel={() => setEditingContractorId(null)} onSave={() => setEditingContractorId(null)} onDelete={() => deleteContractor(contractor.id)} onChange={(field, value) => updateContractor(contractor.id, field, value)} />)}</div></GlassCard></div><GlassCard><SectionHeader icon={<UserPlus className="h-6 w-6" />} title="Add Contractor" subtitle="Save full contractor profile, required contact info, and abilities." /><div className="space-y-3"><div className="grid gap-3 md:grid-cols-2"><Input label="First Name *" value={newContractor.firstName} onChange={(v) => setNewContractor({ ...newContractor, firstName: v })} /><Input label="Last Name *" value={newContractor.lastName} onChange={(v) => setNewContractor({ ...newContractor, lastName: v })} /></div><Input label="Primary Role" value={newContractor.role} onChange={(v) => setNewContractor({ ...newContractor, role: v })} /><Input label="Phone *" value={newContractor.phone} onChange={(v) => setNewContractor({ ...newContractor, phone: v })} /><Input label="Email *" value={newContractor.email} onChange={(v) => setNewContractor({ ...newContractor, email: v })} /><Input label="Company" value={newContractor.company} onChange={(v) => setNewContractor({ ...newContractor, company: v })} /><div className="grid gap-3 md:grid-cols-2"><Input label="City" value={newContractor.city} onChange={(v) => setNewContractor({ ...newContractor, city: v })} /><Input label="State" value={newContractor.state} onChange={(v) => setNewContractor({ ...newContractor, state: v })} /></div><div className="grid gap-3 md:grid-cols-2"><Input label="Emergency Contact Name" value={newContractor.emergencyContactName} onChange={(v) => setNewContractor({ ...newContractor, emergencyContactName: v })} /><Input label="Emergency Contact Phone" value={newContractor.emergencyContactPhone} onChange={(v) => setNewContractor({ ...newContractor, emergencyContactPhone: v })} /></div><label className="block"><span className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-500">Skills / Abilities</span><textarea value={newContractor.skillsText} onChange={(e) => setNewContractor({ ...newContractor, skillsText: e.target.value })} placeholder={`Examples:
A1, A2, V1, V2, L1, L2, Stagehand, LED Tech, Playback, RF Tech`} className="min-h-[110px] w-full rounded-2xl border border-white/10 bg-black/30 p-3 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-amber-400/60" /></label><label className="block"><span className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-500">Notes</span><textarea value={newContractor.notes} onChange={(e) => setNewContractor({ ...newContractor, notes: e.target.value })} className="min-h-[90px] w-full rounded-2xl border border-white/10 bg-black/30 p-3 text-sm text-white outline-none" /></label><Input label="Default Rate" type="number" value={newContractor.rate} onChange={(v) => setNewContractor({ ...newContractor, rate: v })} /><Select label="Rate Type" value={newContractor.rateType} onChange={(v) => setNewContractor({ ...newContractor, rateType: v as "day" | "hour" })} options={[{ value: "day", label: "Day Rate" }, { value: "hour", label: "Hourly" }]} /><GoldButton onClick={addContractor}><UserPlus className="h-4 w-4" /> Save Contractor</GoldButton></div></GlassCard></div>}
...
function EditableContractorCard({ contractor, isEditing, onEdit, onCancel, onSave, onDelete, onChange }: { contractor: Contractor; isEditing: boolean; onEdit: () => void; onCancel: () => void; onSave: () => void; onDelete: () => void; onChange: (field: keyof Contractor, value: string) => void }) { return <div className="rounded-3xl border border-white/10 bg-black/25 p-5"><div className="mb-4 flex items-start justify-between gap-3"><div className="flex-1">{isEditing ? <div className="grid gap-3 md:grid-cols-2"><Input label="First Name" value={contractor.firstName} onChange={(v) => onChange("firstName", v)} /><Input label="Last Name" value={contractor.lastName} onChange={(v) => onChange("lastName", v)} /></div> : <><h3 className="text-xl font-bold">{contractor.name}</h3><p className="text-sm text-zinc-400">{contractor.role}</p></>}</div><ActionButtons isEditing={isEditing} onEdit={onEdit} onCancel={onCancel} onSave={onSave} onDelete={onDelete} /></div>{isEditing ? <div className="grid gap-3"><Input label="Primary Role" value={contractor.role} onChange={(v) => onChange("role", v)} /><Input label="Phone" value={contractor.phone} onChange={(v) => onChange("phone", v)} /><Input label="Email" value={contractor.email} onChange={(v) => onChange("email", v)} /><Input label="Company" value={contractor.company} onChange={(v) => onChange("company", v)} /><div className="grid gap-3 md:grid-cols-2"><Input label="City" value={contractor.city} onChange={(v) => onChange("city", v)} /><Input label="State" value={contractor.state} onChange={(v) => onChange("state", v)} /></div><div className="grid gap-3 md:grid-cols-2"><Input label="Emergency Contact Name" value={contractor.emergencyContactName} onChange={(v) => onChange("emergencyContactName", v)} /><Input label="Emergency Contact Phone" value={contractor.emergencyContactPhone} onChange={(v) => onChange("emergencyContactPhone", v)} /></div><Input label="Rate" type="number" value={String(contractor.rate)} onChange={(v) => onChange("rate", v)} /><Select label="Rate Type" value={contractor.rateType} onChange={(v) => onChange("rateType", v)} options={[{ value: "day", label: "Day Rate" }, { value: "hour", label: "Hourly" }]} /><Input label="Skills (comma separated)" value={contractor.skills.join(", ")} onChange={(v) => onChange("skills", v)} /><label className="block"><span className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-500">Notes</span><textarea className="min-h-[90px] w-full rounded-2xl border border-white/10 bg-black/30 p-3 text-sm text-white outline-none" value={contractor.notes} onChange={(e) => onChange("notes", e.target.value)} /></label><Select label="Status" value={contractor.status} onChange={(v) => onChange("status", v)} options={[{ value: "Active", label: "Active" }, { value: "Inactive", label: "Inactive" }]} /></div> : <div className="mt-5 space-y-3 text-sm"><InfoLine icon={<DollarSign className="h-4 w-4" />} label="Default Rate" value={`${money(contractor.rate)} / ${contractor.rateType}`} /><InfoLine icon={<Phone className="h-4 w-4" />} label="Phone" value={contractor.phone || "Not added"} /><InfoLine icon={<Mail className="h-4 w-4" />} label="Email" value={contractor.email || "Not added"} /><InfoLine icon={<Building2 className="h-4 w-4" />} label="Location / Company" value={`${contractor.city || ""}${contractor.city && contractor.state ? ", " : ""}${contractor.state || ""}${contractor.company ? ` · ${contractor.company}` : ""}` || "Not added"} /><div className="rounded-2xl bg-black/20 p-3"><div className="text-xs text-zinc-500">Skills / Abilities</div><div className="mt-2 flex flex-wrap gap-2">{contractor.skills?.length ? contractor.skills.map((skill, i) => <span key={i} className="rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-1 text-xs text-amber-200">{skill}</span>) : <span className="text-zinc-400">No skills added</span>}</div></div></div>}</div>; }
