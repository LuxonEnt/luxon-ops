...
export default function LuxonOpsDashboard() {
  ...
  async function loadData() {
    ...
    if (contractorError || eventError || assignmentError) {
      ...
      alert(
        "Supabase could not load data. Check your table names, policies, and environment variables."
      );
    }
    ...
    setNewAssignment((prev) => ({
      ...prev,
      eventId: loadedEvents[0]?.id || prev.eventId || 0,
      contractorId: loadedContractors[0]?.id || prev.contractorId || 0,
    }));
  ...
  async function deleteEvent(id: number) {
    const ok = window.confirm(
      "Delete this event and all crew assignments attached to it?"
    );
    if (!ok) return;
  ...
  async function geofenceClock(id?: number, type: "in" | "out" = "in") {
    ...
    try {
      ...
      const locationStamp = `${userLat.toFixed(6)},${userLon.toFixed(
        6
      )} (${Math.round(distance)} ft from event)`;
      ...
      setGeoMessage(
        `Clock-${type} approved. Location verified within ${Math.round(
          distance
        )} ft of event.`
      );
    } catch (error: any) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white">
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute left-[-120px] top-[-160px] h-[360px] w-[360px] rounded-full bg-amber-500/20 blur-[110px]" />
        <div className="absolute right-[-140px] top-[20%] h-[420px] w-[420px] rounded-full bg-yellow-300/10 blur-[130px]" />
    ...
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      ...
      {isEditing ? (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Input label="Address for Google Maps" value={event.address} onChange={(v) => onChange("address", v)} />
          <Input label="Latitude" value={event.latitude} onChange={(v) => onChange("latitude", v)} />
          <Input label="Longitude" value={event.longitude} onChange={(v) => onChange("longitude", v)} />
          <Input
          ...
          />
          <Input label="Start Date" type="date" value={event.startDate} onChange={(v) => onChange("startDate", v)} />
          <Input label="End Date" type="date" value={event.endDate} onChange={(v) => onChange("endDate", v)} />
          <Select
            label="Status"
            value={event.status}
    ...
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex-1">
          {isEditing ? (
            <div className="grid gap-3 md:grid-cols-2">
              <Input label="First Name" value={contractor.firstName} onChange={(v) => onChange("firstName", v)} />
              <Input label="Last Name" value={contractor.lastName} onChange={(v) => onChange("lastName", v)} />
            </div>
          ) : (
            <>
      ...
      {isEditing ? (
        <div className="grid gap-3">
          <Input label="Primary Role" value={contractor.role} onChange={(v) => onChange("role", v)} />
          ...
          <Input label="Company" value={contractor.company} onChange={(v) => onChange("company", v)} />

          <div className="grid gap-3 md:grid-cols-2">
            <Input label="City" value={contractor.city} onChange={(v) => onChange("city", v)} />
            <Input label="State" value={contractor.state} onChange={(v) => onChange("state", v)} />
          </div>
          ...
          <Input label="Rate" type="number" value={String(contractor.rate)} onChange={(v) => onChange("rate", v)} />
    ...
    <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
      ...
      {isEditing ? (
        <div className="grid gap-3 md:grid-cols-3">
          ...
          />
          <Input label="Position" value={row.position} onChange={(v) => onChange("position", v)} />
          <Input label="Work Date" type="date" value={row.workDate} onChange={(v) => onChange("workDate", v)} />
          <Input label="Call Time" type="time" value={row.callTime} onChange={(v) => onChange("callTime", v)} />
          <Input label="Clock In" type="time" value={row.clockIn} onChange={(v) => onChange("clockIn", v)} />
          <Input label="Clock Out" type="time" value={row.clockOut} onChange={(v) => onChange("clockOut", v)} />
          <Input
            label="Break Hours"
            type="number"
          ...
          />
          <Input label="Rate" type="number" value={String(row.rate)} onChange={(v) => onChange("rate", v)} />
          <Select
            label="Rate Type"
            value={row.rateType}
            onChange={(v) => onChange("rateType", v)}
        ...
        <div className="grid gap-3 text-sm md:grid-cols-4">
          <InfoLine icon={<CalendarDays className="h-4 w-4" />} label="Date" value={dateLabel(row.workDate)} />
          <InfoLine
            icon={<Clock className="h-4 w-4" />}
            label="Call / Clock"
      ...
      {showPayrollActions && onToggle && (
        <div className="mt-4 flex flex-wrap gap-2">
          <SmallButton onClick={() => onToggle(row.id, "confirmed")}>Toggle Confirmed</SmallButton>
          <SmallButton onClick={() => onToggle(row.id, "approved")}>Toggle Approved</SmallButton>
